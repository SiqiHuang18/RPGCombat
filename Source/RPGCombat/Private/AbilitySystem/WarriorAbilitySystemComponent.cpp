...
void UWarriorAbilitySystemComponent::RemoveGrantedHeroWeaponAbilities(
	UPARAM(ref) TArray<FGameplayAbilitySpecHandle>& InSpecHanldesToRemove)
{
	if (InSpecHanldesToRemove.IsEmpty())
	{
		return;
	}

	for (const FGameplayAbilitySpecHandle& SpecHandle : InSpecHanldesToRemove)
	{
		if(SpecHandle.IsValid())
			ClearAbility(SpecHandle);
	}

	InSpecHanldesToRemove.Empty();
}
